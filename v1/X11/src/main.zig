...
pub fn main() !void {
    ...
    const Sequence = struct {
        ...
        const status = if(keycode == x11.XKeysymToKeycode(display, x11.XStringToKeysym(Cache.bindedKey)) and Socket.toggle == true and toggle.status == false and std.mem.eql(u8, @"type", "KeyPress")) true || false;
    };
    try network.init();
    ...
    var backend = try sdl.initWindow(.{
        ...
        .vsync = true,
        .title = "Keybinder's X11 Keybinder",
    ...
    main_loop: while(true) {
        ...
        if(std.mem.eql(u8, event.type, "KeyPress")) {
            if(std.mem.eql(u8, event.type, x11.XKeysymToKeycode(display, x11.XStringToKeysym(Cache.toggleKey)))) Sequence.toggle.counter += 1;
